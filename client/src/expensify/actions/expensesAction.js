    ...
    database.ref("expenses").on(
      ...
      (snapshot) => {
        ...
        const filteredExpenses = expenses.filter(
          (expense) => expense.owner === authUser.uid
        );
      ...
      (err) => {
        ...
        dispatch(
          setGlobalError({
            message:
              "permission denied, could not retrieve expenses, please try again later",
            duration: 86400000,
          })
        );
        reject(err);
  ...
  database
    ...
    .catch((err) => {
      ...
      dispatch(
        setGlobalError({
          message:
            "permission denied, could not add expense, please try again later",
          duration: 86400000,
        })
      );
      dispatch(setLoading());
  ...
  database
    ...
    .catch((err) => {
      ...
      dispatch(
        setGlobalError({
          message:
            "permission denied, could not delete expense, please try again later",
          duration: 86400000,
        })
      );
      dispatch(setLoading());
