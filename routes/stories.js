...
router.get("/", async (req, res) => {
  const stories = await Story.find({ user: req.user._id }).sort("-updatedAt");
...
router.put("/:id", validateEditFields, async (req, res) => {
  ...
  if (req.body.storyImage) {
    storage.bucket("storybook_uploads").file(prevFileName).delete();
  }
  ...
  (story.allowComments = !!req.body.allowComments), await story.save();
...
router.delete("/:id", async (req, res) => {
  ...
  const forceDelete = async () => {
    if (story.comments.length) {
      const allComments = story.comments.map((id) =>
  ...
  try {
    await storage.bucket("storybook_uploads").file(storyImage).delete();
...
router.post("/:id/comments/", async (req, res) => {
  ...
  if (!req.body.body.trim()) {
    ...
    return res.render("stories/full_story", {
      story,
      pageTitle: "Full Story",
      errors,
    });
  }
...
router.patch("/:id/:status", async (req, res) => {
  const updatedStory = await Story.findByIdAndUpdate(
    req.params.id,
    { status: req.params.status === "public" ? "private" : "public" },
    { new: true }
  );
  res.redirect("/stories");
